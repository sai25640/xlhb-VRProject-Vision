				...
				if (Rain) {
						if(Refractive_Rain){
							for (int i=0; i<RefractRainParticle.Count; i++) {
								Make_Particles_Appear (RefractRainParticle[i].gameObject, RefractRainParticleP[i], null, SkyManager.Refract_Rain_max_part, true, 12, 1);//v3.2
							}
						}else{
...
//								}
								...
								if (Current_Time <= 24f) {
									...
									if (Lerp_sky_rot && (Current_Time < 3.64f)) {
										MoonCenterObj.transform.rotation = Quaternion.Lerp (MoonCenterObj.transform.rotation, Quaternion.Euler (-Moon_Altitude, Moon_Azimuth, Rot_Sun_Z), Time.deltaTime * (0.21f+(Current_Time*1.6f)));
									}
									else {
										MoonCenterObj.transform.rotation = Quaternion.Euler (-Moon_Altitude, Moon_Azimuth, Rot_Sun_Z);
									}
