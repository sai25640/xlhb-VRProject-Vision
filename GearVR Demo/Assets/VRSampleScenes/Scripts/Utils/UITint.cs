...
namespace VRStandardAssets.Utils
{
    ...
    public class UITint : MonoBehaviour
    {
        ...
        [SerializeField] private Text[] m_TextsToTint;
        ...
        {
            for (int i = 0; i < m_TextsToTint.Length; i++)
            {
                //Debug.Log("Text:" + m_TextsToTint[i].color);
                m_TextsToTint[i].color += m_Tint * m_TintPercent;
            }

            // When the user looks at the VRInteractiveItem go through all the images...
            for (int i = 0; i < m_ImagesToTint.Length; i++)
            {
                //Debug.Log("Image:" + m_ImagesToTint[i].color);
